        ...
        mnLastRelocFrameId(0)                               //恢复为0,没有进行这个过程的时候的默认值
{
    ...
    K.at<float>(0,2) = cx;
    K.at<float>(1,2) = cy;
    K.copyTo(mK);   //copyTo未重新分配内存，clone()重新分配内存
...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im,const double &timestamp)
{
    ...
    if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET) //没有成功初始化的前一个状态就是NO_IMAGES_YET
        mCurrentFrame = Frame(
            mImGray,
            timestamp,
            mpIniORBextractor,      //初始化ORB特征点提取器会提取2倍的指定特征点数目 2000
            mpORBVocabulary,
            mK,
            mDistCoef,
            mbf,
        ...
        mCurrentFrame = Frame(
            mImGray,
            timestamp,
            mpORBextractorLeft,     //正常运行的时的ORB特征点提取器，提取指定数目特征点 1000
            mpORBVocabulary,
            mK,
            mDistCoef,
            mbf,
...
// 正常情况：运动模式TrackWithMotionModel，节省时间;
// 出现意外：重定位模式Relocalization;
// 实在不行：关键帧模式TrackReferenceKeyFrame;
void Tracking::Track()
{
    // track包含两部分：估计运动、跟踪局部地图
    ...
    else
    {
        ...
        // mbOnlyTracking等于false表示正常SLAM模式（定位+地图更新）VO，mbOnlyTracking等于true表示仅定位模式
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                {
                    // 对于上述两个条件的理解：
                    // 第一个条件，如果运动模型为空，那么就意味着之前已经跟丢了
                    // 第二个条件，如果当前帧紧紧跟在重定位的帧后面，那么我们可以认为，
                    // 通过当前帧和发生重定位的帧（作为参考关键帧？）的基础上，我们可以恢复得到相机的位姿
...
void Tracking::MonocularInitialization()
{
    ...
    else    //如果单目初始化器已经被创建
    {
        ...
        // Step 3 在mInitialFrame与mCurrentFrame中找匹配的特征点对
        ORBmatcher matcher(
            0.9,        //最佳的和次佳特征点评分的比值阈值，这里是比较宽松的，跟踪时一般是0.7，分数越低匹配越好
                        //分数即为汉明距离，距离越小匹配越好。最佳<次佳*0.9才算好的匹配，阈值越小，说明特征点的辨识度越好。
            true);      //检查特征点的方向
...
//（3) 当前帧跟踪到大于50个点；
//（4）当前帧跟踪到的比参考关键帧少90%。多余90就删掉，但少于90就会插入关键帧 
